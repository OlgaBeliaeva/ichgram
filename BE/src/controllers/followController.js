...
export const getUserFollowing = async (req, res) => {
  try {
    const following = await Follow.find({ follower_user_id: req.params.userId }).populate('follower_user_id', 'username');
...
export const followUser = async (req, res) => {
  ...
  try {
    ...
    const notification = new Notification({ 
      ...
      type: "Follow",
      // content: `${user.username} follow to you`,
...
export const unfollowUser = async (req, res) => {
  ...
  try {
    const follow = await Follow.findOne({ user_id: new ObjectId(userId), follower_user_id: new ObjectId(targetUserId) });
    console.log(follow)
    if (!follow) {
    ...
    const notification = new Notification({ 
      user_id: targetUser.id,
      type: "Unfollow",
      // content: `${user.username} unfollow from you`,
