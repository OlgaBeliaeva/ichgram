...
export const getOtherUserPosts = async (req, res) => {
  try {
    const { user_id } = req.params; // Извлечение userId из параметров маршрута
    const posts = await Post.find({ user_id: user_id }); // Поиск постов по user_id
    res.status(200).json(posts);
  } catch (error) {
    res.status(500).json({ error: 'Ошибка при получении постов' });
  }
...
export const getFollowingPosts = async (req, res) => {
  try {
    const userId = getUserIdFromToken(req); // ID текущего пользователя
    ...
    const posts = await Post.find({ user_id: { $in: followingIds } })
      ...
      .sort({ created_at: -1 }); // Сортировка по дате

    res.status(200).json(posts);
  } catch (error) {
    res.status(500).json({ error: 'Ошибка при получении постов подписанных пользователей' });
  }
};